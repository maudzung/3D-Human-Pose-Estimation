...
def show3Dpose(channels, ax, lcolor="#3498db", rcolor="#e74c3c", add_labels=False):  # blue, orange
    ...
    assert channels.size == len(h36m_keypoints) * 3, "channels should have 96 entries, it has %d instead" % channels.size
    vals = np.reshape(channels, (len(h36m_keypoints), -1))
...
def show2Dpose(channels, ax, lcolor="#3498db", rcolor="#e74c3c", add_labels=False):
    ...
    assert channels.size == len(h36m_keypoints) * 2, "channels should have 64 entries, it has %d instead" % channels.size
    vals = np.reshape(channels, (len(h36m_keypoints), -1))
